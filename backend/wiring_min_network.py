...
class Model(object):
    def __init__(self, params):
        ...
        self.N_grid = np.sqrt(self.N_rec).astype(int)
        ...
        self.location = np.array(np.meshgrid(range(self.N_grid),range(self.N_grid))).reshape([2,self.N_grid**2]).T
        self.dist_mat = 1.+squareform(pdist(self.location))
        ...
        with tf.variable_scope("model"):
            ...
            self.loss = self.reg_loss() #self.error + self.regularization()
    ...
    def dist_regularizer(self):
        return self.phi*tf.reduce_mean(tf.abs(self.W_rec)*self.dist_mat)
    ...
    def train(self, sess, generator,
        ...
        states = sess.run([self.states], feed_dict={self.x: batch_x})
        ...
        return (t2 - t1), states, [batch_x, batch_y]
