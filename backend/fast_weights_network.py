...
class Model(object):
    def __init__(self, params):
        ...
        with tf.variable_scope("model"):
            ...
            # Network loss
            # ------------------------------------------------
            self.predictions, self.states, self.fast_weights = self.compute_predictions()
    ...
    def compute_predictions(self):
        ...
        for rnn_input in rnn_inputs:
            state,fast_weight = self.rnn_step(rnn_input, state,fast_weight)
            output = self.rnn_output(state)
            rnn_outputs.append(output)
            rnn_states.append(state)
            rnn_fast_weights.append(fast_weight)
        return tf.transpose(rnn_outputs, [1, 0, 2]), rnn_states, rnn_fast_weights
    ...
    def train(self, sess, generator,
        ...
        fast_weights,states,phi,gamma = sess.run([self.fast_weights,self.states,self.phi,self.gamma], feed_dict={self.x: batch_x})
        ...
        return (t2 - t1), fast_weights, phi, gamma, states,[batch_x,batch_y]
