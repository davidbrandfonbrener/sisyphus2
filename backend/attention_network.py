...
class Model(object):
    def __init__(self, params):
        ...
        self.L2_attention = params.get('L2_attention',0)
        ...
        self.gamma_train = params.get('gamma_train',True) #time constant of fast weights
        ...
        with tf.variable_scope("model"):
            ...
            if self.load_weights_path is None:
                ...
                W_att_initializer = tf.random_normal_initializer(mean=0.1, stddev=0.01)
                ...
                gamma_initializer = tf.constant_initializer(self.gamma_init)
            else:
                ...
                W_att_initializer = tf.constant_initializer(weights['W_att'])
                ...
                gamma_initializer = tf.constant_initializer(weights['gamma'])
            ...
            self.W_att = tf.get_variable('W_att',[N_rec,N_in],
                                         initializer=W_att_initializer,
                                         trainable=True)
            ...
            self.gamma = tf.get_variable('gamma', [1], initializer=gamma_initializer,
                                         trainable=self.gamma_train)

            # ------------------------------------------------
            # Network loss
            # ------------------------------------------------
            self.predictions, self.states, self.attention = self.compute_predictions()
    ...
    def regularization(self):
        ...
        reg += self.L2_attention * tf.reduce_mean(tf.square(self.attention))

        return reg

    # implement one step of the RNN
    def rnn_step(self, rnn_in, state, attention):
        ...
        new_state = ((1-self.alpha) * state)\
                        ...
                        + tf.matmul(rnn_in*tf.nn.softmax(attention),self.W_in,transpose_b=True, name="2")\
                        + self.b_rec\
                        + np.sqrt(2.0 * self.alpha * self.rec_noise * self.rec_noise)\
                          * tf.random_normal(state.get_shape(), mean=0.0, stddev=1.0))
          
        #update attention
        new_attention = (1-self.gamma) * attention + self.gamma * tf.matmul(new_state,self.W_att)

        return new_state,new_attention
    ...
    def compute_predictions(self):
        ...
        attention = tf.zeros(self.N_in)
        rnn_outputs = []
        rnn_states = []
        rnn_attention = []
        for rnn_input in rnn_inputs:
            state,attention = self.rnn_step(rnn_input, state, attention)
            output = self.rnn_output(state)
            rnn_outputs.append(output)
            rnn_states.append(state)
            rnn_attention.append(attention)
        return tf.transpose(rnn_outputs, [1, 0, 2]), rnn_states, rnn_attention
    ...
    def train(self, sess, generator,
        ...
        while step * batch_size < training_iters:
            ...
            if step % display_step == 0:
                ...
                loss,phi,gamma,grad = sess.run([self.loss,self.phi,self.gamma,self.grad_W],
        ...
        attention,states,phi,gamma = sess.run([self.attention,self.states,self.phi,self.gamma], feed_dict={self.x: batch_x})
        ...
        if save_weights_path is not None:
            np.savez(save_weights_path, W_in = self.W_in.eval(session=sess),
                                    ...
                                    W_att = self.W_att.eval(session=sess),
                                    ...
                                    gamma=self.gamma.eval(session=sess))


        return (t2 - t1), attention, phi, gamma, states,[batch_x,batch_y]
