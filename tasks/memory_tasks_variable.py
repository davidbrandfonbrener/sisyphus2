...
def build_train_trials(params):
    ...
    task_idx = np.random.randint(low=0, high=len(tasks),size=sample_size)
    ...
    for sample in np.arange(sample_size):
        ...
        #rule input
        x_train[sample,:,2+task_idx[sample]] = 1
        ...
        #task for this trial
        task = tasks[task_idx[sample]]

        if task == 'xor':
            output_pattern = (np.sum(input_pattern[sample,:]) == 1).astype('int')  # xor
        elif task == 'or':
            output_pattern = (np.sum(input_pattern[sample,:]) >= 1).astype('int')  # or
        elif task == 'and':
            output_pattern = (np.sum(input_pattern[sample,:]) >= 2).astype('int')  # and
        elif task == 'memory_saccade':
            output_pattern = input_pattern[sample, 0]  # output=first input
        elif task == 'direct_report':
            output_pattern = input_pattern[sample, 1] #output = second input
        elif task == 'xnor':
            output_pattern = (np.sum(input_pattern[sample,:]) != 1).astype('int')  # xnor
        elif task == 'nor':
            output_pattern = (np.sum(input_pattern[sample,:]) == 0).astype('int')  # nor
        elif task == 'nand':
            output_pattern = (np.sum(input_pattern[sample,:]) < 2).astype('int')  # nand
        elif task == 'memory_saccade_inv':
            output_pattern = np.abs(1-input_pattern[sample, 0])  # output=opposite of first input
        elif task == 'direct_report_inv':
            output_pattern = np.abs(1-input_pattern[sample, 1]) #output = opposite of second input
        

        y_train[sample,out_period,output_pattern] = 1
